...
**파라미터 타입 생략**은 sorted라는 메소드를 정의할 때 이미 파라미터 타입을 정했기 때문에 클로저를 사용할 때는 파라미터명만 명시하고 파라미터 타입은 생략할 수 있다.
```
...
어차피 클로저 내에서 정해진 파라미터는 클로저 내에서만 유효한 변수이기 때문에 간단한 코드만 구현할 것이라면 파라미터명을 생략하고 단축인자를 사용할 수 있다.   
```
