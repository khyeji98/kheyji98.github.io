...
약한 참조는 참조하는 인스턴스 강하게 유지하지 않는 참조로 다른 인스턴스의 수명이 짧을 것 같을 때 사용하는데, 약한 참조는 참조가 되어도 ARC에 추적되지 않아 참조 카운팅이 증가하지 않는다.   
참조하는 인스턴스를 강하게 유지하지 않기 때문에 약한 참조를 하는 동안 해당 인스턴스가 할당 해제될 수 있고, 할당 해제되면 ARC는 자동으로 약한 참조를 nil로 설정한다.   
이렇게 약한 참조는 nil이 할당될 수 있기 때문에 상수가 아닌 옵셔널 타입의 변수로 선언되어야 한다.
...
또한 Person의 강한 참조가 모두 사라지는 것은 물론이고, Apartment의 강한 참조도 하나가 사라져 Apartment의 참조 카운트가 1이 된다.
```
