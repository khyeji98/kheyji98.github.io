...
- 불명확 타입은 제네릭의 반대
  - **제네릭**은 함수를 구현할 때 추상화된 타입을 이용하고, 호출할 때 타입을 구체적으로 지정(T는 호출할 때 타입이 명확해짐)
  - **불명확 타입**은 함수를 구현할 때 구체적인 타입을 이용하고, 호출할 때 추상화된 프로토콜 타입을 이용(some 키워드를 사용해 구현부에서는 명확한 타입 지정) -> 외부에 실제 타입을 숨길 수 있음
- 프로토콜 타입을 반환하면 각종 동작이 제한됨, 그리고 프로토콜에 연관 타입을 지정한다면 반환 타입으로 사용하는 것 자체가 불가능
- 제네릭과 불명확 타입을 같이 쓸 수도 있음
- **불명확 타입을 반환하는 함수의 제약** : 언제나 같은 타입을 리턴해야 한다.(if문같은 것을 사용해 반환 타입을 여러가지로 나누는 것 불가능)
